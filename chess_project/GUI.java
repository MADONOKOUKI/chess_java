...
public class GUI extends JFrame implements ActionListener
{
  ...
  private VerifySurrenderPopup surrender;
  private VerifyRestartgamePopup restart;
  ...
  public void verifySurrender()
  {
    surrender = new VerifySurrenderPopup(this);
    setEnabled(false);
    surrender.setVisible(true);
  }
  public void verifyRestartgame()
  {
    restart = new VerifyRestartgamePopup(this);
    setEnabled(false);
  ...
  {
    //パラメーターの値で使用できるか出来ないかを決定する
    setEnabled(true);
    //to release resources and free up memory.
    promotion.dispose();
  }
  ...
  public void actionPerformed(ActionEvent event)
  {
    ...
    Boolean turn=true;//"turn" is a flag for receiving and it is return value from processOne
    
    if( 80>command && command>= 0)
    {
      int x = command%10;
      int y = command/10;
      System.out.println("do command "+ command);
      Location loc = processLocation(new Location(y,x)); 
      runner.processOne(loc,true);
      System.out.println("自分のターン");
    }
    else{
      if(command == 80){
        runner.processTwo(-12);
      }
      System.out.println("do command "+ command);
